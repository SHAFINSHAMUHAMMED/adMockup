...
function Fb() {
    ...
    const handleClickOutside = (event) => {
        if (activeDropdown && !event.target.closest('.nav-icon')) {
          setActiveDropdown(null);
        }
      };
  
      // Set up event listener for detecting clicks outside dropdown
      useEffect(() => {
    ...
    return (
      <div className="navbar fb-main" onClick={(e) => e.stopPropagation()}>
        <div className="nav-icons flex justify-end pt-20 pb-2 gap-5">
          {/* Icon 1 */}
          <div className="nav-icon" onClick={() => handleIconClick('icon2')}>
          ...
          {/* Icon 2 */}
          <div className="nav-icon" onClick={() => handleIconClick('icon1')}>
          <svg width={25} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" transform="rotate(270)"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4zm-6 0a2 2 0 1 0 0-4 2 2 0 0 0 0 4zm12 0a2 2 0 1 0 0-4 2 2 0 0 0 0 4z" fill="#0D0D0D"></path></g></svg>
            {activeDropdown === 'icon1' && (
              <div className="dropdown">
                <h2>dfdsfdfd</h2>
                <h2>dfdsfdfd</h2>
